var DarknetApp = {
    ...
    switchView: function(viewName) {
        ...
        let activeBtn = document.querySelector(`.nav-btn[onclick="DarknetApp.switchView('${viewName}')"]`);
        if(activeBtn) activeBtn.classList.add('active');

        // Loading state
        document.getElementById('content-area').innerHTML = '<div style="text-align:center; padding:20px; color:#555;">Načítání dat...</div>';
    ...
    onMessage: function(event) {
        ...
        if(data.action === 'darknet_updateData') {
            ...
            if(data.view === 'system') DarknetApp.renderSystemJobs(data.systemJobs, data.activeJob);
            if(data.view === 'market') DarknetApp.renderMarket(data.marketJobs);
            if(data.view === 'myjobs') DarknetApp.renderMyJobs(data.postedJobs, data.activeJob);
    ...
    renderSystemJobs: function(jobs, activeJob) {
        ...
        for (let [id, job] of Object.entries(jobs)) {
            ...
            let btnAttr = isLocked ? 'disabled' : `onclick="System.pluginAction('darknet', 'acceptSystemJob', {jobId: '${id}'})"`;
            let btnText = isLocked ? `ZAMČENO (Rep: ${job.minReputation})` : 'PŘIJMOUT KONTRAKT';

            html += `
                <div class="job-card ${lockedClass}" style="opacity: ${isLocked ? 0.5 : 1}">
                    <div class="job-title">${job.label} <span style="float:right; font-size:10px; color:#00b894">+$${job.payout || 0}</span></div>
                    <div class="job-desc">${job.description}</div>
                    <div class="job-reward">Odměna Reputace: +${job.repReward}</div>
                    <button class="btn" ${btnAttr} style="width:100%">${btnText}</button>
                </div>
            `;
        }
        
        if(html === '') html = '<div style="text-align:center; padding:20px; color:#555;">Žádné další dostupné zakázky.</div>';
            ...
            jobs.forEach(job => {
                html += `
                    <div class="job-card unlocked">
                        <div class="job-title">${job.title} <span class="job-price">$${job.reward}</span></div>
                        <div class="job-desc">${job.description}</div>
                        <div style="font-size:9px; color:#555;">ID: ${job.id} | Zadavatel: ${job.creator_serial.slice(-4)}</div>
                        <button class="btn" onclick="System.pluginAction('darknet', 'acceptCustomJob', {jobId: ${job.id}})">PŘIJMOUT</button>
                    </div>
                `;
            });
        }
    ...
    renderMyJobs: function(posted, active) {
        let html = '<h3 style="color:#aaa; border-bottom:1px solid #333">AKTIVNÍ PRÁCE</h3>';
        ...
        if(active) {
            ...
            if (active.isSystemJob) {
                ...
                statusLabel = '<span style="color:#ff7675; font-size:10px;">(SYSTÉMOVÁ MISE)</span>';
                ...
                statusLabel = '<span style="color:#00b894; font-size:10px;">(P2P KONTRAKT)</span>';
            ...
            html += '<div style="color:#555; margin-bottom:20px;">Nemáš aktivní zakázku.</div>';
        }

        html += '<h3 style="color:#aaa; border-bottom:1px solid #333">MNOU VYPSANÉ</h3>';
        if(posted && posted.length > 0) {
            posted.forEach(job => {
                let statusColor = job.status === 'active' ? '#00b894' : '#e17055';
                let statusText = job.status === 'active' ? 'PROBÍHÁ' : 'ČEKÁ NA PŘIJETÍ';
                let chatBtn = job.status === 'active' ? `<button onclick="System.pluginAction('darknet', 'fetchChat', {jobId: ${job.id}})" style="background:#fff; color:black; border:none; cursor:pointer; padding:2px 5px;">CHAT</button>` : '';
                ...
                html += `
                    <div style="background:#111; border-left:3px solid ${statusColor}; padding:10px; margin-bottom:5px;">
                        ...
                        <div style="margin-top:5px; font-size:10px;">
                            ${chatBtn}
                            <button class="btn-delete" onclick="System.pluginAction('darknet', 'finishCustomJob', {jobId: ${job.id}})">UKONČIT</button>
                        </div>
                    </div>
                `;
            });
    ...
    createJob: function() {
        ...
        if(t && d && r) {
            System.pluginAction('darknet', 'createCustomJob', {title:t, description:d, reward:r});
        }
    },
    ...
    sendMessage: function() {
        ...
        if(val && this.activeChatJobId) {
            System.pluginAction('darknet', 'sendChat', {jobId: this.activeChatJobId, message: val});
            input.value = '';
        }
    },
